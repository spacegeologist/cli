...
交付前快速回看：
- **叙述是否被列举化**：背景 / 现状 / 认识 / 分析 / 成效 / 过渡 / 总结等应成段；列举只留给真正并列的具体问题 / 措施 / 步骤 / 清单。若每个方面 / 每节都机械"一是 / 二是 / 三是"、几乎没有叙述段落，给背景 / 认识 / 分析 / 过渡补上段落（纯清单 / 台账类除外）。
- **数据是否正确呈现**：成行成列的数据应改成表格，不要写成段落或"A+B+C"串。
- **标题是否滥用**："小标题 + 一句话"的小项不要升成标题；按 §二改成标签行 / 加粗引导句段落。
